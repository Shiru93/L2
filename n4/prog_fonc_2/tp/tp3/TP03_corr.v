...
Proof.
  intros hA hABF hB.
  exfalso.
  apply hABF; assumption.
Qed.

(** À vous pour les suivants : *)
Lemma exo_exfalso2 (A B C : Prop) : A \/ B -> (A -> False) -> (B -> False) -> C.
Proof.
  intros [hA | hB] hnA hnB; exfalso; [apply hnA | apply hnB]; assumption.
Qed.
...
Proof.
  intros [h | h] hnA; [exfalso; apply hnA |]; assumption.
Qed.

Lemma not_ex3 (A B : Prop) : (A -> B) -> (~ B -> ~ A).
Proof.
  intros h hnB hA; apply hnB, h; exact hA.
Qed.

Lemma DeMorgan1 (A B : Prop) : ~ (A \/ B) -> ~ A /\ ~ B.
Proof.
  intros h; split; intros hn; apply h; [left | right]; exact hn.
Qed.

Lemma DeMorgan2 (A B : Prop) : ~ A /\ ~ B -> ~ (A \/ B).
Proof.
  intros [hna hnb] [? | ?]; [apply hna | apply hnb]; assumption.
Qed.

Lemma DeMorgan3 (A B : Prop) : ~ A \/ ~ B -> ~ (A /\ B).
Proof.
  intros [h | h] [hA hB]; apply h; assumption.
Qed.
...
Proof.
  exact (fun '(conj hA hnA) => False_ind B (hnA hA)).
Qed.
...
Proof.
  intros [? | ?] hAB; [left; intros ?; apply hAB; split | right]; assumption.
Qed.
...
Proof.
  intros [h | h] hnnA; [exact h | exfalso; apply hnnA; exact h].
Qed.
...
Proof.
  intros [h | h] hAB.
  - left; exact h.
  - right; intros hA; apply h, hAB; exact hA.
Qed.
...
Proof.
  unfold dne, lem.
  intros H; apply H; intros h.
  apply DeMorgan1 in h as [hnA hnnA]; apply hnnA; exact hnA.
Qed.
