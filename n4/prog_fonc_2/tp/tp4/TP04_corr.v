...
Proof.
  intros b b'.
  destruct b as [|], b' as [|]; simpl; reflexivity.
Qed.
...
Proof.
  intros b b'.
  specialize (andb_comm b true) as h1.
  rewrite h1.
  specialize (andb_true_l b) as h2.
  rewrite h2.
  specialize (andb_comm b b') as h3.
  exact h3.
Qed.
...
Proof.
  destruct exo_exists as [b Hb].
  exists b.
  intros b'.
  specialize (andb_comm b' b) as h.
  rewrite h.
  specialize (Hb b').
  exact Hb.
Qed.
...
Proof.
  destruct exo_exists2 as [b Hb].
  exists b.
  intros b'.
  specialize (orb_comm b' b) as h.
  rewrite h.
  specialize (Hb b').
  exact Hb.
Qed.
...
Proof.
  exact (fun h => conj
    (fun x => let '(conj hP _) := h x in hP)
    (fun x => let '(conj _ hQ) := h x in hQ)).
Qed.

Lemma and_forall_forall_and :
  (forall x, P x) /\ (forall x, Q x) -> forall x, P x /\ Q x.
Proof.
  intros [hP hQ] x; split; [exact (hP x) | exact (hQ x)].
Qed.

Lemma and_forall_forall_and' :
  (forall x, P x) /\ (forall x, Q x) -> forall x, P x /\ Q x.
Proof.
  exact (fun '(conj hP hQ) x => conj (hP x) (hQ x)).
Qed.

Lemma or_forall_forall_or :
  (forall x, P x) \/ (forall x, Q x) -> (forall x, P x \/ Q x).
Proof.
  intros [h | h] x; [left | right]; exact (h x).
Qed.
...
Proof.
  exact (fun hpq x =>
    match hpq with
    | or_introl h => or_introl (h x)
    | or_intror h => or_intror (h x)
    end).
Qed.
...
Proof.
  intros [[x Hx] | [x Hx]]; exists x; [left | right]; exact Hx.
Qed.
...
Proof.
  exact (fun h =>
    match h with
    | or_introl (ex_intro x hP) => ex_intro (fun x => P x \/ Q x) x (or_introl hP)
    | or_intror (ex_intro x hP) => ex_intro (fun x => P x \/ Q x) x (or_intror hP)
    end).
Qed.
...
Proof.
  intros [x [h | h]]; [left | right]; exists x; exact h.
Qed.
...
Lemma exists_or_or_exists' :
  (exists x, P x \/ Q x) -> (exists x, P x) \/ (exists x, Q x).
Proof.
  exact (fun '(ex_intro x h) =>
    match h with
    | or_introl hP => or_introl (ex_intro P x hP)
    | or_intror hQ => or_intror (ex_intro Q x hQ)
    end).
Qed.
